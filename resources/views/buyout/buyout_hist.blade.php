	...
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="buyoutHistory">Buyout History <span class="text-muted font-weight-sm"><mark><small>Fully
								Paid Projects</small></mark></span></h5>
						...
						<tbody>
							...
							@foreach ($projects as $projectDetails)
								@if ($projectDetails->bo_companies->count())
									@if ($projectDetails->bo_companies[0]->balance == 0)
										<tr id='project{{ $projectDetails->id }}'>
											...
											<td>{{ $projectDetails->bo_companies[0]->company_name }}</td>
											<td>{{ $projectDetails->bo_companies[0]->total_amt }}</td>
