										...
										<td id="tdActionId{{ $user->id }}" data-id="{{ $user->id }}">
											@if ($user->status == 1)
												...
												@if (!in_array(1, $user->user_roles->pluck('role_id')->toArray()))
													<button type="button" title="Deactivate" id="{{ $user->id }}"
														onclick="disableUser({{ $user->id }})" class="btn btn-danger btn-rounded btn-icon">
						...
						success: function(data) {
							swalWithBootstrapButtons.fire(
								...
								document.getElementById("tdActionId" + dataID).innerHTML =
									...
									")' data-toggle='modal' data-target='#editUser" + id +
									"' class='btn btn-primary btn-rounded btn-icon' title='Edit'><span class='ti-pencil'></span></button> <button type='button' id='action" +
									id + "' onclick='disableUser(" + id +
									")' class='btn btn-danger btn-rounded btn-icon' title='Deactivate'><span class='ti-trash'></span></button> <button type='button' id='reset" +
